    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            if self.atPoint(location).name == "Back_button" || self.atPoint(location).name == "Back_label" {
                ...
                let scene = GameScene(size: self.size, countOfFallingNumbers: 1, fallingSpeed: 80, interval: 0, countOfRow:4,  level: "kid", timerForHelp: 30)
                presentNewScene(scene, skView: skView, button: btn!)
            } else if self.atPoint(location).name == "Easy_button" || self.atPoint(location).name == "Easy_label" {
                let interval = ((self.size.height - 48) / 90) * 20
                print(interval)
                let btn = self.childNode(withName: "Kid's mode_button")
                let scene = GameScene(size: self.size, countOfFallingNumbers: 2, fallingSpeed: 90, interval: Int(interval), countOfRow: 5, level: "easy", timerForHelp: 60)
                presentNewScene(scene, skView: skView, button: btn!)
              
            } else if self.atPoint(location).name == "Hard_button" || self.atPoint(location).name == "Hard_label" {
                let interval = self.size.height / 25
                let btn = self.childNode(withName: "Kid's mode_button")
                let scene = GameScene(size: self.size, countOfFallingNumbers: 2, fallingSpeed: 120, interval: Int(interval), countOfRow: 5, level: "hard", timerForHelp:60)
                presentNewScene(scene, skView: skView, button: btn!)
